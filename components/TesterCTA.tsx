...
const TesterCTA: React.FC<TesterCTAProps> = ({ tone, content }) => {
  ...
  return (
    <section id="tester" className="py-12">
      <div className={`p-8 md:p-16 rounded-[3rem] text-center relative overflow-hidden transition-all duration-500 ${
        tone === 'bold'
        ...
        <div className="max-w-3xl mx-auto space-y-8 relative z-10">
          <h2 className={`text-4xl md:text-6xl font-bold tracking-tighter ${tone === 'bold' ? 'font-bold-brand uppercase text-white' : 'text-white'}`}>
            {content.title}
          </h2>
          
          <div className="flex flex-wrap justify-center gap-4">
            {content.requirements.map((req, idx) => (
              <span key={idx} className={`px-4 py-2 rounded-full text-sm font-semibold border ${
                tone === 'bold' 
                ? 'bg-zinc-950 border-zinc-800 text-zinc-400' 
                : 'bg-white/10 border-white/20 text-blue-50'
              }`}>
                {tone === 'bold' ? <i className="fa-solid fa-check text-blue-500 mr-2"></i> : <i className="fa-solid fa-star mr-2"></i>}
                {req}
              </span>
            ))}
          </div>
          ...
          <div className="space-y-6 pt-4">
            <p className={`text-lg ${tone === 'bold' ? 'text-zinc-500' : 'text-blue-100 opacity-80'}`}>
              Estamos abrindo o app para quem quer ajudar a moldar o futuro das validações. Baixe o app e prove seu ponto.
            </p>
