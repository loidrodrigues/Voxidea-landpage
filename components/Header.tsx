...
const Header: React.FC<HeaderProps> = ({ tone, toggleTone }) => {
  ...
  return (
    <nav className={`sticky top-0 z-50 backdrop-blur-md border-b transition-colors duration-500 ${tone === 'bold' ? 'bg-zinc-950/80 border-zinc-800' : 'bg-white/80 border-slate-200'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center gap-2">
            <div className={`w-8 h-8 flex items-center justify-center rounded-lg ${tone === 'bold' ? 'bg-blue-600' : 'bg-blue-600'}`}>
              <i className="fa-solid fa-lightbulb text-white"></i>
            </div>
            <span className={`text-xl font-bold tracking-tight ${tone === 'bold' ? 'font-bold-brand text-2xl uppercase text-white' : 'text-slate-900'}`}>
              Voxidea
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <i className={`fa-solid ${tone === 'bold' ? 'fa-toggle-on text-blue-500' : 'fa-toggle-off text-blue-500'}`}></i>
